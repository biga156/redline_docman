...
class DocumentsRepository extends ServiceEntityRepository
{
    ...
    */
    ...
    public function findDocumentsByTags($date, $tags)
    {
        ...
        $qb
            ->where('document.date = :date')
            ->leftjoin('documents.tags', 't')
            ->andWhere('t.id IN (:tags)')
            ->setParameters(['date' => $date, 'tags' => $tags]);

        return $qb->getQuery()->getResult();
    }

    //TODO:query for filter by user
    //*select * from documents, users where documents.user_id={id}
